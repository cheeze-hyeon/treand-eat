...
import { FOOD_STORES, getFoodById, getFoodIdByMenuName } from '../data/foods';
...
function resolveContext(storeId: string | null, foodId: string | null) {
 if (storeId && STORES[storeId]) {
 const store = STORES[storeId];
 const resolvedFoodId = getFoodIdByMenuName(store.menuName) ?? foodId ?? '2';
 return { store, food: getFoodById(resolvedFoodId) };
 }

 if (foodId) {
 const food = getFoodById(foodId);
 const topStore = FOOD_STORES[food.id]?.[0];
 const store = topStore ? STORES[topStore.storeId] ?? DEFAULT_STORE : DEFAULT_STORE;
 return { store, food };
 }

 return { store: STORES['1'], food: getFoodById('2') };
}
...
export default function WriteReviewPage() {
 ...
 const { store, food } = resolveContext(
 ...
 const canSubmit =
 satisfied !== null && waitTime !== null && visitDay !== null && visitTime !== null;
 ...
 <p className="font-bold text-[#2e211c] text-xl">
 {store.name}
 </p>
 ...
 <p className="font-bold text-lg text-[#2e211c]">
 트렌딜리셔스
 </p>
 ...
 <p className="font-bold text-lg text-[#2e211c] mb-3">
 텍스트 리뷰
 </p>
 <textarea
 value={text}
 ...
 />
 <p className="mt-2 text-xs text-right text-[#665a55]">{text.length}/500</p>
 </section>

 <section className="rounded-2xl border-2 border-[#2e211c] p-4">
 <p className="font-bold text-lg text-[#2e211c] mb-1">
 사진 업로드
 </p>
