...
const outPath = join(root, 'src/app/data/macaoCrackCookie.generated.ts');
...
const storeItemsText = readFileSync(
  join(dataDir, 'trendeat_macao_crack_cookie_store_items.csv'),
  'utf8',
);
const reviewsText = readFileSync(
  join(dataDir, 'trendeat_macao_crack_cookie_reviews.csv'),
  'utf8',
);
...
const storeEntries = storeItems.map((item) => {
  ...
  return {
    ...
    location: STORE_LOCATIONS[id] ?? '서울 성수동',
    ...
    map: {
      ...
      categories: ['마카오크랙쿠키'],
      position: MAP_POSITIONS[id] ?? { xPercent: 75, yPercent: 40 },
      ...
      reviewCount: parseNum(item.review_count),
      district: STORE_LOCATIONS[id] ?? '서울 성수동',
    },
    foodStore: {
      storeId: id,
...
// Auto-generated by scripts/generate-macao-data.mjs — do not edit by hand.
...
export const MACAO_CRACK_COOKIE_STORE_META: Record<string, MacaoStoreMeta> = {
${metaBlocks}
};
...
console.log(`Wrote ${outPath} (${storeEntries.length} stores, ${reviewRows.length} reviews)`);
